...
// ── Tauri commands ─────────────────────────────────────────────────
// All 23 #[tauri::command] handlers live in [`commands`].  Helper types
// and private functions that only serve the commands were moved there
// as well.  The `generate_handler!` macro in [`run`] references them
// via `commands::*`.
        ...
        .on_menu_event(|app, event| match event.id().as_ref() {
            ...
            "tray_toggle_mascot" => {
                ...
                if mascot_native_window_is_open() {
                    if let Err(err) = commands::mascot_window_hide(app.clone()) {
                        log::error!("[tray] failed to hide mascot window: {err}");
                    }
                } else if let Err(err) = commands::mascot_window_show(app.clone()) {
...
pub fn run() {
    ...
    let daemon_mode = commands::is_daemon_mode();
    ...
    let builder = builder
        ...
        .manage(commands::PendingAppUpdateState::default());
    ...
    builder
        .setup(move |app| {
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                let missing: Vec<&str> = XDG_BINARIES
                    .iter()
                    .copied()
                    .filter(|name| !commands::path_has_executable(name))
                    .collect();
                if missing.is_empty() {
                    if let Err(err) = app.deep_link().register_all() {
                        log::warn!("[deep-link] register_all failed (non-fatal): {err}");
        ...
        .invoke_handler(tauri::generate_handler![
            commands::core_rpc_url,
            commands::core_rpc_token,
            commands::overlay_parent_rpc_url,
            commands::process_diagnostics_list_owned,
            commands::check_core_update,
            commands::apply_core_update,
            commands::check_app_update,
            commands::apply_app_update,
            commands::download_app_update,
            commands::install_app_update,
            commands::restart_core_process,
            commands::recover_port_conflict,
            commands::start_core_process,
            commands::reset_local_data,
            commands::app_quit,
            commands::restart_app,
            commands::get_active_user_id,
            commands::schedule_cef_profile_purge,
            ...
            commands::activate_main_window,
            native_notifications::show_native_notification,
            commands::mascot_window_show,
            commands::mascot_window_hide,
...
mod tests {
    ...
    fn is_daemon_mode_detects_daemon_flag() {
        ...
        let _result = commands::is_daemon_mode();
    }
    ...
    fn overlay_parent_rpc_url_handles_empty() {
        ...
        assert!(commands::overlay_parent_rpc_url().is_none());
    ...
    fn reset_local_data_windows_file_lock_error_codes_are_recognized() {
        assert!(commands::is_windows_file_lock_raw_os_error(Some(32)));
        assert!(commands::is_windows_file_lock_raw_os_error(Some(33)));
        assert!(!commands::is_windows_file_lock_raw_os_error(Some(5)));
        assert!(!commands::is_windows_file_lock_raw_os_error(None));
    }

    #[test]
    fn reset_local_data_delete_error_keeps_generic_message_for_other_errors() {
        let err = std::io::Error::from(std::io::ErrorKind::PermissionDenied);
        let msg = commands::reset_local_data_delete_error(
    ...
    fn no_app_update_available_result_is_quiet_unavailable() {
        let info = commands::no_app_update_available("0.53.43".to_string());
    ...
    fn path_has_executable_finds_file_on_path() {
        ...
        assert!(
            commands::path_has_executable("xdg-mime"),
    ...
    fn path_has_executable_returns_false_for_partial_xdg_utils_install() {
        ...
        assert!(
            !commands::path_has_executable("update-desktop-database"),
            "partial xdg-utils install must NOT report update-desktop-database present (OPENHUMAN-TAURI-5V)"
        );
        assert!(
            !commands::path_has_executable("xdg-icon-resource"),
